...
var outView *walk.WebView
var tableView *walk.TableView
var searchButton *walk.Button
...
func GetResult() (col *walk.TableViewColumn) {
	col = walk.NewTableViewColumn()
	col.SetName("name")
	col.SetDataMember("dataMember")
	return
}
		...
		HSplitter{
			Children: []Widget{
				PushButton{
					...
					OnClicked: func() {
						s := "*" + inTE.Text() + "*"
						res := es.Search( s, "green")
						o := es.FormatResult(res)
						s1 := ""
						for _, h := range o.Hits.Hits {
							s1 = s1 + h.Source.Filename + "\n"
							s1 = s1 + h.Highlight.Content[0] + "\n"
						}
						outView.SetToolTipText(s1)
					},
				},
				TextEdit{AssignTo: &inTE, MaxSize: Size{100, 20}},
			}, MaxSize: Size{100, 20},
		},
		WebView{AssignTo: &outView, URL: "https://www.baidu.com"},
	},
}
